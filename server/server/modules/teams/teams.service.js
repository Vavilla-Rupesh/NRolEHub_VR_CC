...
//     // Check for duplicate team names in the same event/subevent
//     const existingTeam = await Team.findOne({
//       where: {
//         name: teamData.name.trim(),
//         event_id: teamData.event_id,
//         subevent_id: teamData.subevent_id
//       }
//     });
...
//     },
//     attributes: ['id', 'name', 'leader_id', 'event_id', 'subevent_id', 'created_at'],
//     include: [{
//       model: TeamMember,
...
//       include: [{
//         model: Team,
//         attributes: ['id', 'name', 'leader_id', 'event_id', 'subevent_id'],
//         include: [{
//           model: TeamMember,
//           where: { status: 'accepted' },
//           include: [{
...
exports.markTeamAttendance = async (teamId, present = false, adminId) => {
  ...
  try {
    ...
    if (team.TeamMembers.length < subevent.min_team_size) {
      throw new Error(
        `Team must have at least ${subevent.min_team_size} members to mark attendance`
      );
    }
  ...
  try {
    ...
    if (leaderboard.length === 0 && teamsWithAttendance.length > 0) {
      const initialEntries = await Promise.all(
        teamsWithAttendance.map(async (ta) => {
          return TeamLeaderboard.create({
            team_id: ta.team_id,
            event_id: eventId,
            subevent_id: subEventId,
...
exports.editTeamWinners = async (eventId, subEventId, winners) => {
  ...
  try {
    ...
    // Create team leaderboard entries
    const teamLeaderboardEntries = await TeamLeaderboard.bulkCreate(
      leaderboardEntries,
      { transaction }
    );
