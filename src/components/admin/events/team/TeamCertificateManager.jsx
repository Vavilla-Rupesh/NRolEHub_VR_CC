...
import { Search, Award, Download, Upload, Eye, X, CheckCircle, AlertCircle, Trophy, Ban } from 'lucide-react';
...
return;
  ...
  const participationTeams = eligibleTeams.filter(team => !leaderboard.find(lb => lb.rank <= 3 && lb.team_id === team.id));
  const meritTeams = leaderboard.filter(team => team.rank <= 3);
  const currentTeams = activeTemplate === 'merit' ? meritTeams : participationTeams;

  const filteredTeams = currentTeams.filter(team => 
    (team.name || team.Team?.name)?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    (team.TeamMembers || team.Team?.TeamMembers)?.some(member => 
      ...
      <div className="max-w-6xl mx-auto space-y-6 md:space-y-8">
        ...
        <div className="text-center mb-8 animate-fade-in">
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-600 bg-clip-text text-transparent mb-2">
            Team Certificate Manager
          </h1>
          <p className="text-slate-600 dark:text-slate-300 text-sm md:text-base">
            Generate professional team certificates with ease
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 md:gap-4 mb-6 md:mb-8">
          <button
            ...
            className={cn(
              ...
              activeTemplate === 'participation' 
                ? 'bg-gradient-to-r from-emerald-500 via-emerald-600 to-cyan-700 text-white shadow-emerald-500/30' 
          ...
          >
            ...
            <div className="relative flex items-center justify-center space-x-2">
              <Award className="h-4 w-4 md:h-5 md:w-5 transition-transform group-hover:rotate-12" />
            ...
            <div className="absolute inset-x-0 bottom-0 h-1 bg-gradient-to-r from-emerald-400 to-cyan-400 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left"></div>
          </button>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              <p className="text-blue-800 dark:text-blue-200 text-sm md:text-base font-medium leading-relaxed">
                {activeTemplate === 'merit' ? (
                  `${meritTeams.length} teams eligible for merit certificates`
                ) : (
                  `${participationTeams.length} teams eligible for participation certificates`
                )}
              </p>
              <p className="text-blue-600 dark:text-blue-300 text-xs md:text-sm mt-1">
                Ready to generate high-quality team certificates
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="relative flex items-center justify-center space-x-2 md:space-x-3">
            {generating ? (
              <>
                ...
                <span>Generating Team Certificates...</span>
              </>
            ) : isCurrentTemplateGenerated ? (
              <>
              ...
              <>
                <Award className="h-5 w-5 md:h-6 md:w-6 transition-transform group-hover:scale-110 group-disabled:scale-100" />
                <span>Generate {activeTemplate === 'merit' ? 'Merit' : 'Participation'} Team Certificates</span>
              </>
            )}
          </div>
