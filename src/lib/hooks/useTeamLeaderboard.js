  ...
  const fetchLeaderboard = async () => {
    try {
      ...
      if (teamsWithAttendance.length > 0) {
        ...
        // Filter out invalid/empty leaderboard entries and only include entries with valid data
        const validLeaderboardEntries = leaderboardResponse.data.filter(entry => 
          entry.team_id && 
          entry.team_id !== "" && 
          entry.team_id !== null && 
        ...
        if (validLeaderboardEntries.length > 0) {
          // Sort by rank and get top 3
          const leaderboardData = validLeaderboardEntries
            .sort((a, b) => (a.rank || Infinity) - (b.rank || Infinity))
            .slice(0, 3)
            .map((entry) => ({
              ...entry,
